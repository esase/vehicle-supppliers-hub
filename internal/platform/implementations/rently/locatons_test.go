...
func TestLocationsRequest(t *testing.T) {
			...
			t.Run(test.name, func(t *testing.T) {
				...
				handlerFunc = func(w http.ResponseWriter, r *http.Request) {
					...
					if handlerFuncCalledCount == 2 {
						assert.Equal(t, "/api/Places", r.RequestURI)
	...
	t.Run("should return build supplier requests history array", func(t *testing.T) {
		...
		assert.Equal(t, testServer.URL+"/connect/token", *(*locationsResponse.SupplierRequests)[0].RequestContent.Url)
		...
		assert.Equal(t, testServer.URL+"/api/Places", *(*locationsResponse.SupplierRequests)[1].RequestContent.Url)
		assert.Equal(t, http.MethodGet, *(*locationsResponse.SupplierRequests)[1].RequestContent.Method)
		assert.Len(t, *(*locationsResponse.SupplierRequests)[1].RequestContent.Headers, 1)
...
func locationsDefaultConfiguration() schema.RentlyConfiguration {
...
func locationsParamsTemplate(configuration schema.RentlyConfiguration) schema.LocationsRequestParams {
...
func defaultAuthRedisCacheKey(supplierApiUrl string) string {
	return fmt.Sprintf("rently-auth-token:%s-test-username-test-password", supplierApiUrl)
}
