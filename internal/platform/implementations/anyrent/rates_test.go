...
func TestRatesRequest(t *testing.T) {
			...
			t.Run(test.name, func(t *testing.T) {
				...
				handlerFunc = func(w http.ResponseWriter, r *http.Request) {
					...
					if handlerFuncCalledCount == 2 {
						assert.Equal(t, "/v1/prices?driver_age=39&dropoff_date=20230902T123000Z&dropoff_station=MAN&pickup_date=20230901T123000Z&pickup_station=MAN", r.RequestURI)
	...
	t.Run("should return build supplier requests history array", func(t *testing.T) {
		...
		assert.Equal(t, testServer.URL+"/v1/authorize", *(*ratesResponse.SupplierRequests)[0].RequestContent.Url)
		...
		assert.Equal(t, testServer.URL+"/v1/prices?driver_age=39&dropoff_date=20230902T123000Z&dropoff_station=MAN&pickup_date=20230901T123000Z&pickup_station=MAN", *(*ratesResponse.SupplierRequests)[1].RequestContent.Url)
		assert.Equal(t, http.MethodGet, *(*ratesResponse.SupplierRequests)[1].RequestContent.Method)
		assert.Len(t, *(*ratesResponse.SupplierRequests)[1].RequestContent.Headers, 2)
...
func ratesDefaultConfiguration() schema.AnyRentConfiguration {
	return schema.AnyRentConfiguration{
		ApiKey: "test-api-key",
	}
}

func ratesParamsTemplate(configuration schema.AnyRentConfiguration) schema.RatesRequestParams {
